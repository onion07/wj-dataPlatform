...
(function($) {
	...
	jQuery.fn.MyDigitClock = function(options) {
		var id = $(this).data("id");
		var tagId = $(this).attr("id") == id ? true : false;
		...
		function showClock(id)
		{
			...
			templateStr = templateStr.replace("{FY}", getDD(fy));
			templateStr = templateStr.replace("{MON}", getDD(mon));
			templateStr = templateStr.replace("{DD}", getDD(date));
	...
	jQuery.fn.MyDigitClock.defaults = {
		...
		//fontWeight: 'bold',
		//background: '#fff',
		timeFormat: '{FY}.{MON}.{DD}&nbsp;&nbsp;{HH}<span id="ch1">:</span>{MM}<span id="ch2">:</span>{SS}',
