...
var _DA = (function (Ch,Grid){
        ...
        // var URL_profit_rate = 'mockDemo/pro_rate2.json';  //毛利率
        ...
        var APP = {
             ...
             */
            Request : function (arg,callback,fail) {  
           ...
           /**************************  初始化入口  **************************/
            init : function (){ 
                ...
                // 每隔一段时间重新加载数据
                 ...
                 _ts.rlTime = setInterval(_ts.getData.bind(_ts),1000 * 60 * 5);
            },
            // 加载数据、渲染   
            getData : function (o){
                var _ts = this;
                ...
                // 毛利率
                _ts.Request( { _url: URL_profit_rate,_type:'POST',_data: {type:o} }, function (ret) { _ts._profit_rate(ret); ret7 = ret},function (ret){
                        var index = 1,
                            _tabHead = $('#tab-head'),
                            ...
                            _shBtn = $('.f-btn.lk');
                        if (ret.code !== 1) {
                            _tab.removeClass('cur').eq(0).addClass('cur');
                            _tab.eq(1).hide();
                            _shBtn.hide();
                            _tabcn.removeClass('cur').eq(0).addClass('cur');
                        }
                });
                // 健康值
                _ts.Request( { _url: URL_asset_health,_type:'POST',_data: {type:o}  }, function (ret) { _ts._asset_health(ret); });
                // 资产规模
                _ts.Request( { _url: URL_asset_size,_type:'POST',_data: {type:o} }, function (ret) { _ts._asset_size(ret); ret9 = ret;});
            ...
            isShow : function (ret1,ret2,ret3,ret4,ret5,ret6,ret7) {
                    ...
                    if (!ret1 || !ret2 || !ret3 || !ret4 || !ret5 || !ret6) {  console.warn('网速过慢或部分数据请求失败'); /*return false;*/}
                    _btn.click(function () {
                        // 清除所有定时器
                        _ts.clearIntor();
                        //隐藏
            ...
            _scale : function (ret1,is_show_button){
                    ...
                    if (_ts.pro_Lock) { 
                        ...
                        for (var i = 0; i < len; i++) {
                            arr.push(ret1.xdata[i].name);
                            // $('#outer1 .bb em').eq(i).text(arr[i]);
                            _html += '<li><em></em><span class="icon"></span></li>'
            ...
            _time_Line : function (ret5) {
                    ...
                    var mapSeriseData = ret5.series.data;//银行对应数据（测试地址）

                    var setData = function (is_Hide){
                        var index = 0;
                        var data = is_Hide ? mapYdata : mapChg;
                        // console.info('mapSeriseData=', ret5)
            ...
            _time_Line2 : function (ret6){
                    ...
                    var mapSeriseData = ret6.series.data;//银行对应数据（测试地址）
            ...
            _asset_size : function (ret9){
                ...
                for (var i = 0,len = ret9.series[0].data.length; i < len; i++) {
                    // 处理成左侧数据（负数）
                    ret9.series[1].data[i] = ret9.series[1].data[i] * -1;
                    ret9.series[2].data[i] = ret9.series[2].data[i] * -1;
                    ret9.series[5].data[i] = ret9.series[5].data[i] * -1;

                    var leftData = ret9.series[2].data[i] + ret9.series[5].data[i];
                    var rightData = ret9.series[3].data[i] + ret9.series[4].data[i];

                    total_left.push(leftData);
                    total_right.push(rightData);
                ...
                ucsmy_asset_size.setOption({
                    ...
                    xAxis : [{
                        min: _Min,//'万元' 换成 '元'，控制长度 
                        max: _Max,//'万元' 换成 '元'，控制长度 
                    }],
                    series: [
                        {
                        ...
                        // 左侧总和
                        {
                            stack : ret9.series[6].stack,
                            data: total_left
                        },
                        // 右侧总和
                        {
                            stack : ret9.series[7].stack,
                            data: total_right
                        }
                    ]
                });
